...
extern coap_resource_t res_vibration_status;
...
PROCESS_THREAD(vibration_server, ev, data)
{
...
#endif
  printf("Press a button to switch the vibration status\n");
#endif
  ...
  while (1)
  {
    ...
    else if (ev == sensors_event && data == &button_sensor)
    {
      ...
      LOG_INFO("Button pressed: switch the vibration status from %d to %d\n", status, !status);
